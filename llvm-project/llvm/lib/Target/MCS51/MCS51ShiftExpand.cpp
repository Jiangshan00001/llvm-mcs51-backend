//===- MCS51Shift.cpp - Shift Expansion Pass --------------------------------===//
//
...
/// like MCS51-gcc. This must be done in IR because otherwise the type legalizer
...
class MCS51ShiftExpand : public FunctionPass {
public:
  static char ID;

  MCS51ShiftExpand() : FunctionPass(ID) {}

  bool runOnFunction(Function &F) override;

  StringRef getPassName() const override { return "MCS51 Shift Expansion"; }
...
char MCS51ShiftExpand::ID = 0;

INITIALIZE_PASS(MCS51ShiftExpand, "MCS51-shift-expand", "MCS51 Shift Expansion",
                false, false)

Pass *llvm::createMCS51ShiftExpandPass() { return new MCS51ShiftExpand(); }

bool MCS51ShiftExpand::runOnFunction(Function &F) {
...
void MCS51ShiftExpand::expand(BinaryOperator *BI) {
