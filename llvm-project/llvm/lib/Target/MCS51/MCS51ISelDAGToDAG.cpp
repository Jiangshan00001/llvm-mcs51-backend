...
public:
  MCS51DAGToDAGISel(MCS51TargetMachine &TM, CodeGenOpt::Level OptLevel)
      : SelectionDAGISel(TM, OptLevel), Subtarget(nullptr) {}

  StringRef getPassName() const override {
    return "MCS51 DAG->DAG Instruction Selection";
  }
...
bool MCS51DAGToDAGISel::selectIndexedLoad(SDNode *N) {
  ...
  case MVT::i8: {
    ...
    Opcode = (isPre) ? MCS51::LDRdPtrPd : MCS51::LDRdPtrPi;
    break;
  }
  case MVT::i16: {
    ...
    Opcode = (isPre) ? MCS51::LDWRdPtrPd : MCS51::LDWRdPtrPi;
    break;
  }
  default:
    return false;
...
unsigned MCS51DAGToDAGISel::selectIndexedProgMemLoad(const LoadSDNode *LD,
                                                   MVT VT) {
  ISD::MemIndexedMode AM = LD->getAddressingMode();
...
bool MCS51DAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
  ...
  if (RegNode &&
      RI.getRegClass(RegNode->getReg()) == &MCS51::PTRDISPREGSRegClass) {
    OutOps.push_back(Op);
    return false;
  }
  ...
  if (Op->getOpcode() == ISD::ADD || Op->getOpcode() == ISD::SUB) {
    ...
    if (CopyFromRegOp->getOpcode() == ISD::CopyFromReg) {
      ...
      CanHandleRegImmOpt &= (Register::isVirtualRegister(Reg) ||
                             MCS51::PTRDISPREGSRegClass.contains(Reg));
    } else {
    ...
    if (CanHandleRegImmOpt) {
      ...
      if (RI.getRegClass(Reg) != &MCS51::PTRDISPREGSRegClass) {
        SDLoc dl(CopyFromRegOp);

        Register VReg = RI.createVirtualRegister(&MCS51::PTRDISPREGSRegClass);
...
template <> bool MCS51DAGToDAGISel::select<ISD::FrameIndex>(SDNode *N) {
  auto DL = CurDAG->getDataLayout();
  ...
  CurDAG->SelectNodeTo(N, MCS51::FRMIDX,
                       getTargetLowering()->getPointerTy(DL), TFI,
                       CurDAG->getTargetConstant(0, SDLoc(N), MVT::i16));
  return true;
}

template <> bool MCS51DAGToDAGISel::select<ISD::STORE>(SDNode *N) {
  ...
  unsigned Opc = (VT == MVT::i16) ? MCS51::STDWSPQRr : MCS51::STDSPQRr;
...
template <> bool MCS51DAGToDAGISel::select<ISD::LOAD>(SDNode *N) {
  const LoadSDNode *LD = cast<LoadSDNode>(N);
  ...
  Chain = CurDAG->getCopyToReg(Chain, DL, MCS51::R31R30, Ptr, SDValue());
  Ptr = CurDAG->getCopyFromReg(Chain, DL, MCS51::R31R30, MVT::i16,
                               Chain.getValue(1));

  SDValue RegZ = CurDAG->getRegister(MCS51::R31R30, MVT::i16);
  ...
  if (unsigned LPMOpc = selectIndexedProgMemLoad(LD, VT)) {
    ...
    switch (VT.SimpleTy) {
    case MVT::i8:
      ResNode = CurDAG->getMachineNode(MCS51::LPMRdZ, DL, MVT::i8, MVT::Other,
                                       Ptr, RegZ);
      break;
    case MVT::i16:
      ResNode = CurDAG->getMachineNode(MCS51::LPMWRdZ, DL, MVT::i16,
                                       MVT::Other, Ptr, RegZ);
      ReplaceUses(SDValue(N, 1), SDValue(ResNode, 1));
      break;
    default:
...
template <> bool MCS51DAGToDAGISel::select<MCS51ISD::CALL>(SDNode *N) {
  SDValue InFlag;
  ...
  SDLoc DL(N);
  Chain = CurDAG->getCopyToReg(Chain, DL, MCS51::R31R30, Callee, InFlag);
  SmallVector<SDValue, 8> Ops;
  Ops.push_back(CurDAG->getRegister(MCS51::R31R30, MVT::i16));
  ...
  SDNode *ResNode =
    CurDAG->getMachineNode(MCS51::ICALL, DL, MVT::Other, MVT::Glue, Ops);
...
template <> bool MCS51DAGToDAGISel::select<ISD::BRIND>(SDNode *N) {
  SDValue Chain = N->getOperand(0);
  SDValue JmpAddr = N->getOperand(1);

  SDLoc DL(N);
  // Move the destination address of the indirect branch into R31R30.
  Chain = CurDAG->getCopyToReg(Chain, DL, MCS51::R31R30, JmpAddr);
  SDNode *ResNode = CurDAG->getMachineNode(MCS51::IJMP, DL, MVT::Other, Chain);
...
bool MCS51DAGToDAGISel::selectMultiplication(llvm::SDNode *N) {
  SDLoc DL(N);
  ...
  unsigned MachineOp = isSigned ? MCS51::MULSRdRr : MCS51::MULRdRr;
  ...
  if (N->hasAnyUseOfValue(0)) {
    SDValue CopyFromLo =
        CurDAG->getCopyFromReg(InChain, DL, MCS51::R0, Type, InGlue);
  ...
  if (N->hasAnyUseOfValue(1)) {
    SDValue CopyFromHi =
        CurDAG->getCopyFromReg(InChain, DL, MCS51::R1, Type, InGlue);
...
void MCS51DAGToDAGISel::Select(SDNode *N) {
...
bool MCS51DAGToDAGISel::trySelect(SDNode *N) {
  ...
  case MCS51ISD::CALL: return select<MCS51ISD::CALL>(N);
